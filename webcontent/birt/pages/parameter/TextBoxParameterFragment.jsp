...
<TR>
    ...
    <TD NOWRAP>
        <FONT TITLE="<%= parameterBean.getToolTip( ) %>"><LABEL
                FOR="<%= encodedParameterName %>"><%= parameterBean.getDisplayName() %>:</LABEL></FONT>
        <%-- is required --%>
        <%
    ...
    <TD NOWRAP WIDTH="100%">
        ...
        <INPUT CLASS="BirtViewer_parameter_dialog_Input"
            ...
            <%if (encodedParameterName.indexOf("date")>=0) {%>
               onblur="javascript:jQuery(this).show()"
               onfocus="javascript:jQuery('#<%= encodedParameterName %>').datepicker({format: 'yyyy-mm-dd'});"
            <%}%>>
        <% {%>
        <%} %>
